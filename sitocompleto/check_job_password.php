<?php
    ...
    $password = mysqli_real_escape_string($conn, $_GET['q1']);
    $email = mysqli_real_escape_string($conn, $_GET['q2']);
    #costruisco la query
    $query = "SELECT * FROM impiegato WHERE email='$email'";
    $res = mysqli_query($conn, $query);

    if(mysqli_num_rows($res) > 0){
        $entry = mysqli_fetch_assoc($res);
        #creo l'array da convertire in json che devo ritornare
        $json_array = array('exists_password' => password_verify($password, $entry['password'])? true: false);
